...
const getClaimTableColumns = (role: UserRole, type: ClaimType, category: ClaimCategory) => {
    if (role === USER_ROLES.USER && type === CLAIM_TYPES.MISC && category === CLAIM_CATEGORY.HISTORY) {
        return ['date', 'purpose', 'status', 'amount', 'actions'];
    }
    else if (role === USER_ROLES.USER && type === CLAIM_TYPES.OUTPATIENT && category === CLAIM_CATEGORY.HISTORY) {
        ...
        return ['date', 'name', 'relationship', 'amount', 'actions'];
    }
